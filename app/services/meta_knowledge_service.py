...
class MetaKnowledgeService:
    ...
    async def _save_columns_to_qdrant(self, column_infos: list[ColumnInfo]):
        ...
        for index in range(0, len(embedding_texts), embedding_batch_size):
            batch_embedding_texts = embedding_texts[index:index + embedding_batch_size]
            ...
            for column in table.columns:
                if column.sync:
                    ...
                    current_values_infos = [ValueInfo(id=f"{table.name}.{column.name}.{current_column_value}",
                                                      ...
                                                      column_id=f"{table.name}.{column.name}") for current_column_value
                                            in
    ...
    async def build(self, config_path: Path):
        ...
        if meta_config.tables:
            ...
            # 2.3 对指定的纬度字段取值建立全文索引
            await self._save_values_to_es(meta_config)
            logger.info("为指定的纬度字段取值建立全文索引成功")
